minetest.register_abm({  -- Controls the contained fires.
	nodenames = {'furniture_mod:fireplace', 'furniture_mod:fireplace_on'},
	interval = 1.0,
	chance = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if meta:get_float('fuel_time') < meta:get_float('fuel_totaltime') then
			...
			minetest.swap_node(pos, {name = 'furniture_mod:fireplace_on', param2 = node.param2})
			...
			if fuel.time <= 0 then
				...
				if node.name == 'furniture_mod:fireplace_on' then
					meta:set_string('infotext','Put more wood in the fireplace!')
					minetest.swap_node(pos, {name = 'furniture_mod:fireplace', param2 = node.param2})
					meta:set_string('formspec', furniture_mod.fireplace_formspec)
